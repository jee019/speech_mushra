...
GenericAudioControl.prototype.initAudio = function() {
  ...
  if (maxChannels === 1 && deviceChannels >= 2) {
    channelCount = 2;
...
GenericAudioControl.prototype.process = function(audioProcessingEvent) {
  ...
  if (this.audioCommand === 'switch') {
    ...
    for ( channel = 0; channel < outputBuffer.numberOfChannels; ++channel) {
      ...
      var srcChannelSwitch = Math.min(channel, audioBufferSwitch.numberOfChannels - 1);
      inputData = audioBuffer.getChannelData(srcChannel);
      var inputDataSwitch = audioBufferSwitch.getChannelData(srcChannelSwitch);
