...
MushraAudioControl.prototype.initAudio = function() {
  ...
  // Ensure mono files play in both ears by outputting stereo and duplicating the mono channel.
  // ScriptProcessorNode with 1 output channel may map to left only on some systems.
  var refChannels = this.reference.getAudioBuffer().numberOfChannels;
  var deviceChannels = this.audioContext.destination.channelCount || 2;
  var channelCount = null;
  if (refChannels === 1 && deviceChannels >= 2) {
    channelCount = 2;
  } else if (refChannels > 2) {
    channelCount = deviceChannels;
  } else {
    channelCount = refChannels;
  }
...
MushraAudioControl.prototype.process = function(audioProcessingEvent) {
  ...
  for (channel = 0; channel < outputBuffer.numberOfChannels; ++channel) {
    outputData = outputBuffer.getChannelData(channel);
    // Upmix: if stimulus is mono but output is stereo, duplicate mono channel.
    var srcChannel = Math.min(channel, audioBuffer.numberOfChannels - 1);
    inputData = audioBuffer.getChannelData(srcChannel);
      ...
      if (fadingActive == 1) { // fade out
        ...
        if (fadingCurrentPosition >= this.audioFadingMaxPosition) {          
          ...
          if (this.audioFadingIn === null) {
            ...
            var fadeBuffer = stimulus.getAudioBuffer();
            var fadeSrcChannel = Math.min(channel, fadeBuffer.numberOfChannels - 1);
            inputData = fadeBuffer.getChannelData(fadeSrcChannel);
          }
