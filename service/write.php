...
// Check if trials exist and have likert_single_stimulus type before accessing stimulusRating
$ratingCount = 0;
if (isset($session->trials) && count($session->trials) > 0) {
    foreach ($session->trials as $trial) {
        if ($trial->type == "likert_single_stimulus" && isset($trial->responses) && count($trial->responses) > 0) {
            if (isset($trial->responses[0]->stimulusRating)) {
                $ratingCount = count($trial->responses[0]->stimulusRating);
                break;
            }
        }
    }
